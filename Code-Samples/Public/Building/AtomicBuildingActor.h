...
//
// Generated UArrowComponents / UTextRenderComponents / UBoxComponents
...
public:
	...
	void InitializePlacedBuilding(
		const FGuid& InBuildingInstanceID,
		const FName InBuildingID,
		const FIntVector InAnchorCoord,
		const EBuildingRotation InRotation,
		AAtomicShipGrid* InOwningGrid
	);

	void ConfigureFootprintCollision(
...
protected:
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Atomic|Components")
